CREATE FUNCTION [data].[AmazonRdsSqlServer_TopQueries_QueryWaitStats_Keys_Purgeable]
(
    @PurgeDate AS BIGINT
)
RETURNS TABLE
...
FROM
    [data].[AmazonRdsSqlServer_TopQueries_QueryWaitStats_Keys]
WHERE
...
(
    SELECT
        [Id]
    FROM
        [data].[AmazonRdsSqlServer_TopQueries_QueryWaitStats_Sightings]
    WHERE
