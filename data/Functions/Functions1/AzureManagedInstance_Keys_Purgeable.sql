CREATE FUNCTION [data].[AzureManagedInstance_Keys_Purgeable]
(
    @PurgeDate AS BIGINT
)
RETURNS TABLE
...
SELECT
    [data].[AzureManagedInstance_Keys].[Id]
FROM
    [data].[AzureManagedInstance_Keys]
WHERE
...
(
    SELECT
        [Id]
    FROM
        [data].[AzureManagedInstance_Sightings]
    WHERE
