CREATE FUNCTION [data].[AzureManagedInstance_ServerWaits_Keys_Purgeable]
(
    @PurgeDate AS BIGINT
)
RETURNS TABLE
...
FROM
    [data].[AzureManagedInstance_ServerWaits_Keys]
WHERE
...
(
    SELECT
        [Id]
    FROM
        [data].[AzureManagedInstance_ServerWaits_Sightings]
    WHERE
