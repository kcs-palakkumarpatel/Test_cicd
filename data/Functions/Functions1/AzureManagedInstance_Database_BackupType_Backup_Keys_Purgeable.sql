CREATE FUNCTION [data].[AzureManagedInstance_Database_BackupType_Backup_Keys_Purgeable]
(
    @PurgeDate AS BIGINT
)
RETURNS TABLE
...
FROM
    [data].[AzureManagedInstance_Database_BackupType_Backup_Keys]
WHERE
...
(
    SELECT
        [Id]
    FROM
        [data].[AzureManagedInstance_Database_BackupType_Backup_Sightings]
    WHERE
