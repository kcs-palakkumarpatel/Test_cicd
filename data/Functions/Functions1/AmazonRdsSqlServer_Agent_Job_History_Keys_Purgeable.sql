CREATE FUNCTION [data].[AmazonRdsSqlServer_Agent_Job_History_Keys_Purgeable]
(
    @PurgeDate AS BIGINT
)
RETURNS TABLE
...
FROM
    [data].[AmazonRdsSqlServer_Agent_Job_History_Keys]
WHERE
    [_RunDate] <= @PurgeDate
    AND [Id] NOT IN 
(
    SELECT
        [Id]
    FROM
        [data].[AmazonRdsSqlServer_Agent_Job_History_Sightings]
    WHERE
